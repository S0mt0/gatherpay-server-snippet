...
export class AuthService {
  ...
  async signUp(dto: CreateUserDto, sessionId: string) {
    ...
    await this.cache.set(SIGN_UP_SESSION(dto.phoneNumber), dto, S_ID_TTL);
  ...
  async resendSignUpVerificationCode(sessionId: string) {
    ...
    await this.cache.set(SIGN_UP_SESSION(decrypted), signUpSession, S_ID_TTL);
  ...
  async login(dto: LoginUserDto, deviceInfo: IDeviceInfo) {
    ...
    if (session.twoFactorEnabled && session.twoFactorSecret) {
      await this.cache.set(USER_2FA(user.id), user, S_2FA_TTL);
  ...
  forgotPassword = async (dto: ForgotPasswordDto) => {
    ...
    await this.cache.set(PASSWORD_SESSION(dto.phoneNumber), dto, S_ID_TTL);
  ...
  generate2FASecret(name?: string, length: number = 20) {
